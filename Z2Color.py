...
class Z2ColorBatchNorm(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=6, out_channels=96, kernel_size=11, stride=3, groups=1)
        self.conv1_pool = nn.MaxPool2d(kernel_size=3, stride=2)
        self.conv1_pool_norm = nn.BatchNorm2d(96)

        self.conv2 = nn.Conv2d(in_channels=96, out_channels=256, kernel_size=3, stride=2, groups=2)
        self.conv2_pool = nn.MaxPool2d(kernel_size=3, stride=2)
        self.conv2_pool_norm = nn.BatchNorm2d(256)
        self.ip1 = nn.Linear(in_features=2048, out_features=512)
        self.ip1_norm = nn.BatchNorm1d(512)
        self.ip2 = nn.Linear(in_features=512, out_features=4)
...
def unit_test():
    ...
    # a = test_net(Variable(torch.randn(10, 6, 480, 640)))
    # print (a)
